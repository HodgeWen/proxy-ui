...
<task type="auto">
  ...
  <files>internal/db/db.go, go.mod, go.sum</files>
  <action>
...
- Keep sqlite3store in session.go unchanged — it uses *sql.DB from GORM; when GORM uses glebarez/sqlite, the underlying *sql.DB is already pure-Go, so no CGO

Run: go get github.com/glebarez/sqlite && go mod tidy
- This removes gorm.io/driver/sqlite and mattn/go-sqlite3 (indirect)
- glebarez/sqlite is pure-Go; sqlite3store uses database/sql interface only, no driver dependency
</action>
  <verify>CGO_ENABLED=0 go build ./... succeeds; go test ./... passes; existing s-ui.db works (no schema change)</verify>
