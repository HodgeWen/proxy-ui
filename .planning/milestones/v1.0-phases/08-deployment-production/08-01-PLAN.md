...
must_haves:
  truths:
    - "Build succeeds with CGO_ENABLED=0 (pure-Go SQLite)"
...
<objective>
Migrate to pure-Go SQLite (CGO-free), add panel config file with auto-generate on first run, and HTTPS proxy support (cookie Secure + reverse proxy docs).

Purpose: Enable CGO_ENABLED=0 for Docker and cross-compilation; enable Docker config mount and HTTPS behind reverse proxy per CONTEXT.
Output: CGO-free db layer, internal/config/panel.go, refactored main/api/core to use config, docs/reverse-proxy.md
</objective>
...
<task type="auto">
  <name>Task 1: Migrate to pure-Go SQLite (CGO-free)</name>
  <files>internal/db/db.go, internal/session/session.go, go.mod, go.sum</files>
  <action>
Replace CGO-dependent SQLite with pure-Go driver so CGO_ENABLED=0 builds and cross-compilation work (required by 08-02 Docker and 08-04 GitHub Actions).

In internal/db/db.go:
- Replace import `gorm.io/driver/sqlite` with `github.com/glebarez/sqlite`
- Change `sqlite.Open(path)` to `sqlite.Open(path)` (API is identical; glebarez/sqlite is drop-in)

In internal/session/session.go:
- Replace sqlite3store with an internal store implementing the SCS Store interface using only *sql.DB (no direct go-sqlite3 import).
- SCS Store interface requires: Find(token string) (b []byte, exists bool, err error), Commit(token string, b []byte, expiry time.Time) error, Delete(token string) error, All() (map[string][]byte, error).
- Implement these methods with standard database/sql queries against the sessions table (token TEXT PRIMARY KEY, data BLOB, expiry REAL) — table creation in NewManager stays as-is.
- Remove `github.com/alexedwards/scs/sqlite3store` from imports.
- Assign the new internal store to sm.Store instead of sqlite3store.New(sqlDB).

Run: go get github.com/glebarez/sqlite && go mod tidy
- This removes gorm.io/driver/sqlite, mattn/go-sqlite3, and sqlite3store (go-sqlite3 stays if sqlite3store stays).
- After removing sqlite3store, go mod tidy will remove go-sqlite3 entirely.
</action>
  <verify>CGO_ENABLED=0 go build ./... succeeds; go test ./... passes; existing s-ui.db works (no schema change); ! grep -q go-sqlite3 go.mod (confirms sqlite3store removed)</verify>
  <done>Build succeeds with CGO_ENABLED=0; no go-sqlite3 in go.mod; go test passes</done>
</task>

<task type="auto">
  <name>Task 2: Panel config loader with auto-generate</name>
...
<task type="auto">
  <name>Task 3: Session cookie Secure for HTTPS proxy</name>
...
<task type="auto">
  <name>Task 4: Reverse proxy documentation</name>
